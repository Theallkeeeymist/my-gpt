...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.lists = nn.ModuleList([self.SingleHeadAttention(embedding_dim, head_size) for _ in range(num_heads)])
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        head_output = [head(embedded) for head in self.lists]

        combined = torch.cat(head_output, dim=2)

        return torch.round(combined, decimals=4)
